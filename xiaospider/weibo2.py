...
class Neihan:
    def __init__(self):
        self.start_url = "https://m.weibo.cn/api/container/getIndex?type=uid&value=3267187212&containerid=1076033267187212&since_id=4360257751302636"
    ...
    def get_first_page_content_list(self,html_str): #提取第一页的数据
        ...
        # content_list = re.findall(r"<div article=\"weibo-main\"(.*?)</div>",html_str,re.S)
        content_list = html_str
    ...
    def save_content_list(self,html_str): #保存
        with open("weibo1.txt","a",encoding="utf-8") as f:
            f.write(html_str)
            f.write("\n")
        print("保存成功")
